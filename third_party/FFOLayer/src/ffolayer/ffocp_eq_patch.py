        ...
        def backward(ctx, *dvars):
            ...
            with torch.enable_grad():
                for i in range(B):
                    ...
                    # The new dual term must be evaluated at the perturbed
                    # primal; otherwise parameterized equalities lose their
                    # mixed primal-parameter derivative.
                    eq_new = b["eq_dual_term_torch"](*vars_new_i, *params_i, *new_eq_dual_i)
                    ...
                    # CVXPY represents X >> 0 with Lagrangian term -<Y, X>.
                    # For parameter-dependent PSD expressions, use the full
                    # old/new finite difference with that cone sign.
                    loss = loss + (phi_new + ineq_new + eq_new + exp_new - psd_new - phi_old - eq_old - ineq_old + psd_old)
