...
sparse_solvers = ["scipy SPLU", "scipy MINRES matrix-free"]

from . import set_solver_tolerance
...
def matrix_free_minres(
    ...
    """Solve a symmetric indefinite system exposed as a LinearOperator."""
    iterations = 0
    ...
    x, info = sp.sparse.linalg.minres(
        operator,
    ...
    residual = np.linalg.norm(operator @ x - np.asarray(b).reshape(-1))
    relative_residual = residual / max(np.linalg.norm(b), np.finfo(float).eps)
    return x, {
        "info": info,
